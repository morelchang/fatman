...
$(function() {
	resetPageState();
	$.unblockUI();
	changeRequireLonLatState('<s:property value="requiredLonLat" />');
});
...
function changePageModeState(mode) {
	if (mode == 'CREATE_USER_PROFILE') {
		...
		$('#userSection input').attr('disabled', 'disabled');
		$('#profileSection input,#profileSection select').attr('disabled', 'disabled');
		$('#currentProfile_UePermissionMode').removeAttr('disabled');
		
	} else if (mode == 'CHANGE_LOCATIONDETECTMODE') {
		$('#userSection input').attr('disabled', 'disabled');
		$('#profileSection input,#profileSection select').attr('disabled', 'disabled');
		$('#permissionListSection input').attr('disabled', 'disabled');
		$('input[name="currentProfile\\.locationDetectMode"]').removeAttr('disabled');
...
function changeRequireLonLatState(requireLonLat) {
	if (requireLonLat == 'true') {
		$.blockUI({ message : $('#requireLonlatSection') });
	}
}
...
function addMsisdn(tableElement) {
	...
	var clearButton = '<input type="image" src="<s:url value='/images/buttonClose24.png'/>" />';
...
function validateSave() {
	...
	if ('<s:property value="mode" />' == 'CREATE_USER_PROFILE') {
	...
	message += validateNumber('currentProfile_maxPermissionListSize', '白名單數限制必須為數字');
	message += validateNonEmpty('currentProfile_maxUserCount', '同時最大用戶數為必填');
	message += validateNumber('currentProfile_maxUserCount', '同時最大用戶數必須為數字');
...
function validateUserAccount() {
	var message = '';
	message += validateNumber('current_mobile', '用戶門號必須為數字');
...
function saveWithManualLonLat() {
	if (validateManualLonLat()) {
		return confirm('確定儲存?');
	}
	return false;
}

function validateManualLonLat() {
	var message = '';
	message += validateNumber('manualLon', '經度必須為數字');
	message += validateNumber('manualLat', '緯度必須為數字');
	if (message != '') {
		alert(message);
		return false;
	}
...
<s:form id="addUserForm" namespace="%{#namespace}" action="save" validate="true" theme="simple">
	...
	<tr>
		<td width="440px" valign="top">
			...
			<div id="profileSection" style="width: 94%;" class="grayBorder">
				<span>用戶申裝資訊</span>
				<s:hidden name="currentProfile.oid" />
				<table class="noBorder">
					...
					<tr id="currentProfileStaticIpSection"><td align="right"><s:label>固定IP位址:</s:label></td>
				...
				<table id="permissionListTable" border="0" class="noBorder">
					<tr>
						<td>
							<table id="permissionListTable" class="noBorder" cellspacing="0" cellpadding="0">
								...
								<tr>
									...
									<td><input type="image" src='<s:url value="/images/buttonClose24.png"/>' 
						...
						<td valign="bottom" align="right">
							<input type="image" src='<s:url value="/images/buttonAdd24.png"/>' 
...
<s:form id="leaveForm" namespace="/provision" action="leaveModifyProfile">
</s:form>

<div id="requireLonlatSection" style="display: none">
	<s:form id="requireLonlatForm" namespace="%{#namespace}" action="save" validate="true" theme="simple">
		<s:hidden id="oid" name="current.oid" />
		<s:hidden name="currentProfile.oid" />
		<table cellpadding="5" class="noBorder">
			<tr>
				<td align="left" colspan="2">
					<span>
					輸入地址:<s:property value="currentProfile.address" /><br/>
					<br/>
					輸入的地址無法取得對應的經緯度, 可能格式有誤或其他原因無法換得經緯度<br/>
					(注意：地址【段】可為中文或阿拉伯數字，但X巷、X號、X樓皆需為阿拉伯數字)<br/>
					<br/>
					您可以手動設定經緯度，或點選取消返回修改地址
					</span>
				</td>
			</tr>
			<tr>
				...
				<td align="left">
					<s:textfield id="manualLon" name="manualLon" maxlength="10" size="10"></s:textfield><br/>
				</td>
			</tr>
			<tr>
				<td align="right">
				...
				<td align="left">
					<s:textfield id="manualLat" name="manualLat" maxlength="10" size="10"></s:textfield><br/>
				</td>
			</tr>
			<tr>
				<td align="right" colspan="2">
					<s:submit value="儲存" onclick="javascript: return saveWithManualLonLat();" ></s:submit>
					<input type="button" value="取消" onclick="javascript: $.unblockUI();" />
				</td>
			</tr>
		</table>
	</s:form>
